...
%  STFT will be plotted, in decibels (20 * log10(|S|)).
  ...
  if noverlap > 0
    ...
    stftArr = zeros(ceil(numel(data) / numel(window)) * numel(window), 1);
    stftArr(1 : numel(data)) = data(:);
    stftArr = reshape(stftArr, numel(window), []);
    stftArr = bsxfun(@times, stftArr, window / numel(window));
  end
